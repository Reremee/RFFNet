...
def train(train_loader, model, optimizer, epoch,save_path):
    ...
    try:
        for i, (images, gts, depths) in enumerate(train_loader, start=1):
            ...
            loss1 = CE(s, gts)
            loss2 = CE(_1, gts)/2 + CE(_2, gts)/4 + CE(_3, gts)/8 + CE(_4, gts)/16
            loss = loss1 + loss2
        ...
        loss_all /= epoch_step
        logging.info('#####TRAIN#####')
        logging.info('Epoch [{:03d}/{:03d}], Loss_AVG: {:.4f}'.format(epoch, opt.epoch, loss_all))
        if (epoch) % 10 == 0:
...
def test(test_loader,model,epoch,save_path):
    ...
    with torch.no_grad():
        ...
        for i in range(test_loader.size):
            ...
            image = image.cuda()
            depth = depth.cuda()
            res, _1, _2, _3, _4 = model(image, depth)
        ...
        logging.info('#####VAL######')
        logging.info('Epoch:{} MAE:{} bestEpoch:{} bestMAE:{}'.format(epoch, mae, best_epoch, best_mae))
